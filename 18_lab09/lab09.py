...
def subseqs(s):
    ...
    if s == []:
        ...
        t = subseqs(s[1:])
        return t + insert_into_all(s[0], t)
...
class Account:
    ...
    def __repr__(self):
        return f'Accountholder: {self.holder}, Deposits: {self.dep_num}, Withdraws: {self.wit_num}'
...
def trade(first, second):
    ...
    if m <= len(first) and n <= len(second) and equal_prefix():
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def printer(lnk):
        if lnk is Link.empty:
            ...
            return front + repr(lnk.first) + mid + printer(lnk.rest) + back
    return printer
...
def long_paths(t, n):
    ...
    """
    def dfs(t, l):
        if t.is_leaf() and len(l) > n:
            yield l
        for x in t.branches:
            yield from dfs(x, l + [x.label])
    return list(dfs(t, [t.label]))
