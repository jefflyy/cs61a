...
def about(topic):
    ...
    def check(paragraph):
        for s in [lower(remove_punctuation(s)) for s in split(paragraph)]:
            if s in topic :
                return True
        return False
    return check
...
def accuracy(typed, reference):
    ...
    if len1==0 :
        return 100.0 if len2==0 else 0.0
    tot=0
    for i in range(min(len1,len2)) :
        if typed_words[i]==reference_words[i] :
            tot+=1
    return tot/len1*100.0
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words :
        return typed_word
    ans=min(valid_words,key=lambda x : diff_function(typed_word,x,limit))
    return ans if diff_function(typed_word,ans,limit)<=limit else typed_word
...
def feline_flips(start, goal, limit):
    ...
    if limit<0 :
        return 0
    dif=int(start[0]!=goal[0])
    return dif+feline_flips(start[1:],goal[1:],limit-dif)
...
def minimum_mewtations(start, goal, limit):
    ...
    if len1==0 or len2==0 :
        ...
        # substitute
        dif=int(start[0]!=goal[0])
        ans1=dif+minimum_mewtations(start[1:],goal[1:],limit-dif)
        limit=min(limit,ans1)
        # remove
        ans2=1+minimum_mewtations(start[1:],goal,limit-1)
        limit=min(limit,ans2)
        # add
        ans3=1+minimum_mewtations(start,goal[1:],limit-1)
...
def time_per_word(words, times_per_player):
    ...
    return match(words,[trans(x) for x in times_per_player])
...
def fastest_words(match):
    ...
    n_player=len(player_indices)
    n_word=len(word_indices)
    ans=[[] for i in player_indices]
    for word in word_indices :
        ans[min(player_indices,key=lambda player: time(match,player,word))].append(word_at(match,word))
    return ans
