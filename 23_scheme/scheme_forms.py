...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        val = do_lambda_form(Pair(signature.rest, expressions.rest), env)
        env.define(signature.first, val)
...
def make_let_frame(bindings, env):
    ...
    names = values = nil
    # BEGIN PROBLEM 14
    for x in reversed(to_list(bindings)):
        validate_form(x, 2, 2)
        names = Pair(x.first, names)
        values = Pair(scheme_eval(x.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        validate_form(expressions, 2, 2)  # Checks that expressions is a list of length exactly 2
        # BEGIN PROBLEM 4
        val = MacroProcedure(nil, expressions.rest.first, env)
        ...
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        val = MacroProcedure(signature.rest, expressions.rest.first, env)
        env.define(signature.first, val)
        return signature.first
        # END PROBLEM 10
    else:
