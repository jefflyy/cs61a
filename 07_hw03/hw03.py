...
def num_eights(pos):
    ...
    return 0 if pos==0 else num_eights(pos//10)+(pos%10==8)
...
def pingpong(n):
    ...
    def sign(n):
        if n==0 :
            return 1
        return sign(n-1)*(-1 if (n%8==0 or num_eights(n)>0) else 1)
...
def missing_digits(n):
    ...
    las=n%10
    n//=10
    return missing_digits(n)+max(las-n%10-1,0)
...
def count_coins(change):
    ...
    def count(n,cur):
        if n==0 or cur==1 :
            return 1
        return count(n,descending_coin(cur))+(count(n-cur,cur) if n>=cur else 0)
...
def make_anonymous_factorial():
    ...
    return lambda n,fun=(lambda n,rfun:1 if n==0 else n*rfun(n-1,rfun)):fun(n,fun)
