class VendingMachine:
    ...
    def vend(self):
        ...
        self.balance = 0
        self.stock -= 1
        return f"Here is your {self.name}{f' and ${change} change' if change else ''}."
...
def store_digits(n):
    ...
    l = store_digits(n // 10)
    x = l
    while x.rest is not Link.empty:
        x = x.rest
    x.rest = Link(n % 10)
    return l
...
def two_list(vals, amounts):
    ...
    l = Link.empty
    for v, n in zip(reversed(vals), reversed(amounts)):
        for i in range(n):
            l = Link(v, l)
    return l
...
def bst_min(t):
    ...
    if len(ch) == 2:
        return bst_min(ch[0])
    return bst_min(ch[0]) if ch[0].label <= t.label else t.label
...
def bst_max(t):
    ...
    if len(ch) == 2:
        return bst_max(ch[1])
    return bst_max(ch[0]) if ch[0].label >= t.label else t.label
...
def is_bst(t):
    ...
    ch = t.branches
    if len(ch) == 2:
        return is_bst(ch[0]) and is_bst(ch[1]) and bst_max(ch[0]) <= t.label <= bst_min(ch[1])
    return is_bst(ch[0]) and (bst_max(ch[0]) <= t.label or t.label <= bst_min(ch[0]))
