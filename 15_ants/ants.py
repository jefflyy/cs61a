...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            o_ant = place.ant
            if o_ant.is_container and o_ant.can_contain(self):
                o_ant.store_ant(self)
            elif self.is_container and self.can_contain(o_ant):
                place.ant = self
                self.store_ant(o_ant)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not now.is_hive:
            if self.min_range <= dis <= self.max_range and len(now.bees) > 0:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        r_damage = amount
        if not self.place:
            r_damage += self.damage
        for x in list(place.bees):
            x.reduce_health(r_damage)
        # END Problem 5
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.placed_queen:
            return
        gamestate.placed_queen = True
    ...
    def action(self, gamestate):
        ...
        while now:
            if now.ant:
                buff(now.ant)
                if now.ant.is_container and now.ant.ant_contained:
                    buff(now.ant.ant_contained)
            now = now.exit
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        def back(x):
            return x if x.entrance.is_hive else x.entrance

        destination = back(self.place) if self.scared_cnt else self.place.exit

        # Extra credit: Special handling for bee direction
        if self.blocked():
            print('DEBUG:block')
            self.sting(self.place.ant)
        elif self.health > 0 and destination is not None and (self.slow_time == 0 or gamestate.time % 2 ==0):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not now.is_hive:
            if now.ant:
                if now.ant is not self:
                    res[now.ant] = dis
                if now.ant.is_container and now.ant.ant_contained and now.ant.ant_contained is not self:
                    res[now.ant.ant_contained] = dis
            for x in now.bees:
                res[x] = dis
            now = now.entrance
            dis += 1
