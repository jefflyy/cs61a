...
def prune_min(t):
    ...
    l = t.branches[0]
    r = t.branches[1]
    rm = l if l.label <= r.label else r
    t.branches = [rm]
    prune_min(rm)
...
class Player:
    ...
    def debate(self, other):
        p1 = self.popularity
        p2 = other.popularity
        self.popularity = max(p1 + (50 if random() < max(0.1, p1 / (p1 + p2)) else -50), 0)
    ...
    def speech(self, other):
        ...
        other.popularity = max(p2 - p2 // 10, 0)
...
class Game:
    ...
    def play(self):
        while not self.game_over:
            p1 = [self.p1, self.p2][self.turn]
            p2 = [self.p1, self.p2][1 - self.turn]
            p1.choose(p2)(p2)
    ...
    def winner(self):
        ...
        if p1.votes == p2.votes:
            return None
        return p1 if p1.votes > p2.votes else p2
...
def add_trees(t1, t2):
    ...
    b = t2.branches
    m = min(len(a), len(b))
    ch = [add_trees(a[i], b[i]) for i in range(m)] + a[m:] + b[m:]
    return Tree(t1.label + t2.label, ch)
...
def match_url(text):
    ...
    path = r'/(?:\w+/)*\w+(?:\.\w+|/)'
    anchor = r'#[a-zA-Z0-9_\-]+'
    s = rf"^(?:{scheme})?{domain}(?:{path})?(?:{anchor})?$"
    r = re.match(s, text)
    return bool(r)
