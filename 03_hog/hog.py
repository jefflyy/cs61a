...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    return roll_dice(num_rolls,dice) if num_rolls>0 else picky_piggy(opponent_score)
...
def hog_pile(player_score, opponent_score):
    ...
    return 0 if player_score != opponent_score else player_score
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    now=1
    while score[now]<goal :
        opp=now
        now=next_player(opp)
        num=strategy[now](score[now],score[opp])
        score[now]+=take_turn(num,score[opp],dice,goal)
        score[now]+=hog_pile(score[now],score[opp])
        score0,score1=score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        now_score=[score0,score1][who]
        inc=now_score-last_score
        if(inc>running_high):
            print(inc," point(s)! That's a record gain for Player ",who,"!",sep='')
            return announce_highest(who,now_score,inc)
        else:
            return announce_highest(who,now_score,running_high)
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for cnt in range(1,11):
        t=make_averaged(roll_dice,trials_count)(cnt,dice)
        if(t>best_gain):
            best_gain=t
            best_roll=cnt
    return best_roll
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    t=picky_piggy(opponent_score)
    if t+score==opponent_score :
        return 0
    return 0 if t>=cutoff else num_rolls
